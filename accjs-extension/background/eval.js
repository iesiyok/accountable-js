...
const measurement = async ( tabId, acList, manList, degree, frames_info, topLevel) => {

	if (acList && manList) {

		let resultList = assignDefault2Results(acList.length);

		let statAcL  = 	filterStaticContent(acList);
		let statManL = 	filterStaticContent(manList);
			
		resultList = await compareStatic(tabId, statAcL, statManL, degree, frames_info, topLevel, resultList ) ;
		
	
	
		let dynAcL  = 	filterDynamicContent(acList);
		
		if (dynAcL.length > 0){
			let dynManL = 	filterDynamicContent(manList);
...
const compareInline = async (ac, man) => {
		if (ac.error ){
			return await mismatchStrInline(ac.seq, "Error occurred while reading the script from HTML");
		}else{
			...
			if (ac.script && man.hash){
				...
				if (man.hash == await sha256(ac.script)){
	                ...
	                return await mismatchStrInline(ac.seq, "Hash values mismatch.");
				}

			}else{
				return await mismatchStrInline(ac.seq, "Wrong declaration of inline script, an item is missing.");
			}
...
const compareExternal = async (tabId, ac, man, degree) => {
	...
	if (ac.error){
		return await mismatchStrExternal(ac.seq, ac.link, "Error occurred while reading the script from HTML");
	}else{
		...
		if(man.trust == 'blindtrust'){
			...
			if (sameOrigin(man.link, ac.link)){
				...
				return await mismatchStrExternal(ac.seq, ac.link, "The source of the script in HTML is not in the same origin with whitelisted link" );
			}
			...
			if (ac.hash == DEFAULT_HASH_VALUE){
				...
				return await mismatchStrExternal(ac.seq, ac.link, "Integrity(hash) attribute not found for this script.");
				...
				if (ac.link && man.link && ac.hash && man.hash){
					...
					if (ac.link == man.link && ac.hash == man.hash ){
						...
						return await mismatchStrExternal(ac.seq, ac.link, "Hash values mismatch.");
					}


				}else{
					//return mismatchStr(ac, man);
					return await mismatchStrExternal(ac.seq, ac.link, "Wrong declaration of trust value 'assert' in manifest, one of the items missing" );
				}
			}
			...
			if (delScrManL != null){
				...
				if(ds.length > 0){
					...
					if (delScr.sig_ver){
						...
						return await mismatchStrExternal(ac.seq, ac.link, "Signature not verified.");
					}
			...
			return await mismatchStrExternal(ac.seq, ac.link, "Unknown trust value declared in manifest");
		}
			...
			if(man.src_type == undefined || man.src_type == 'link'){
							...
							if (ac.src == man.src){
								...
								let newFrameInfoEventHandler = (e) => {
									...
									compareInBackground(tabId, ac, e.detail.frame, man, d, e.detail.index);
								...
								if (frames_info != undefined && frames_info != null){
									...
									if(fs.length > 0){
										...
										return compareInBackground(tabId, ac, frame, man, d, index);
